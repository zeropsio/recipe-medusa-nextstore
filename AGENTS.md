...
Official Medusa v2.19 Next.js App Router storefront (from [dtc-starter](https://github.com/medusajs/dtc-starter) `apps/storefront`) on Zerops Node SSR. Yarn **3.2.3**. Talks to [recipe-medusa](https://github.com/zeropsio/recipe-medusa).
...
- Dev command: `yarn dev` (`next dev --turbopack -p 8000`)
...
- SDK: [`src/lib/config.ts`](src/lib/config.ts) accepts `NEXT_PUBLIC_MEDUSA_BACKEND_URL` or `MEDUSA_BACKEND_URL`. Middleware ([`src/middleware.ts`](src/middleware.ts)) needs a reachable Medusa + publishable key at runtime.
